...
$level = $_SESSION['level'];
    ...
    if($_POST['idx']) {
        $id = $_POST['idx'];      
        $sql = $connect->query("SELECT * FROM barang WHERE kode = $id");
        while ($dt =$sql->fetch_object()){ $getsup = $dt->suplierid; 
		?>
      <form action="action/action?act=up-barang" style="font-size: 14px" method="POST" >
        <div class="row form-modal">
          <div class="col-md-4">
            <label for="exampleFormControlInput1">Barcode</label>
            <input type="hidden" class="form-control" name="kode" style="font-size: 12px" value="<?= $dt->kode ?>"readonly>
            <input type="text" class="form-control" name="barcode" style="font-size: 12px" value="<?= $dt->barcode ?>">
          </div>
          <div class="col-md-8">
...
<?php 
$res  = $connect->query("SELECT edit AS edit FROM barang WHERE kode = '$id'");
