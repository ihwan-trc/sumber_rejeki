...
<!-- Form Add Barang -->
  ...
  <div class="modal-dialog modal-lg " role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle" style="font-size: 20px">Form tambah Barang</h5>
        <button type="button" class="close"  data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
      <form action="action/action?act=add-barang" style="font-size: 14px" method="POST">
        <div class="row form-modal">
          <?php 
            $result   = $connect->query('SELECT max(kode) AS kode FROM barang');
            $execute  = $result->fetch_object();   
            $nums_row = $result->num_rows;
          ?>
          <input type="hidden" class="form-control" style="font-size: 12px" value="<?= $execute->kode+1 ?>">

          <div class="col-md-4">
            <label for="exampleFormControlInput1">Barcode</label>
            <input type="text" class="form-control" style="font-size: 12px" name="barcode" required>
          </div>
          <div class="col-md-8">
            <label for="exampleFormControlInput1">Nama Barang</label>
            <input type="text" class="form-control" style="font-size: 12px" name="nama" required>
          </div>
          ...
          <div class="col-md-6">
            ...
            <input type="text" class="form-control" onkeyup="convertToRupiah(this);" style="font-size: 12px" name="hbeli" required>
          </div>
          <div class="col-md-6">
            <label for="exampleFormControlInput1">Harga jual</label>
            <input type="text" class="form-control" onkeyup="convertToRupiah(this);" style="font-size: 12px" name="hjual" required>
          </div>
          <hr width="100%" align="center" color="#0A1B2A">
          <div class="col-md-6">
 ...
 <div class="modal fade" id="edit_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg " role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle" style="font-size: 20px">Form edit Barang</h5>
        <button type="button" class="close"  data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
        $('#edit_modal').on('show.bs.modal', function (e) {
            ...
            $.ajax({
                type : 'post',
                url : 'pages/modul/modal-e-barang.php',
                data :  'idx='+ idx,
