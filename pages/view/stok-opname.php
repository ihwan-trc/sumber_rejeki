...
?>
<?php $res = $connect->query('SELECT * FROM data_toko');
      $sh  = $res->fetch_object() ?>
...
<?php if ($sh->so == 'Buka'): ?>
  ...
  <div class="card-body">
    ...
    <label class="col-sm-2 col-form-label" style="text-align: right;">Pilih Barang</label>
    <div class="col-md-8">
  ...
  <?php if (isset($_POST['kode'])): 
      ...
      $res = $connect->query("SELECT * FROM barang WHERE kode = '$kode' ");
      $dt  = $res->fetch_object();
 
  ?>

         <div class="form-group row">
              <label class="col-sm-2" style="text-align: right;">Data Barang</label>
              <div class="col-md-8">
       ...
       <div class="row" >
        <div class="col-md-6">
          <form action="action/action.php?act=stok-opname" method="POST">
            <div class="col-md-12">
             <div class="form-group row">
              <label class="col-sm-4" style="text-align: right;">Stok Barang</label>
              <div class="col-md-8">
                <input type="hidden" name="kode_barang" value="<?=  $kode ?>">
          ...
          <table id="dataTable" class="table"  width="100%" cellspacing="0" style="font-size: 12px">
            ...
            <tbody>
              ...
              <?php if (isset($_POST['kode'])): 
                  ...
                  $kode = intval($val);
              ?>
              <?php $no=1; $result = $connect->query("SELECT opname.stok, nama, nyata, selisih, keterangan, waktu FROM opname, barang WHERE barang.kode=opname.kode_barang AND kode_barang= '$kode'");
...
<?php else: ?>
  ...
  <div class="card-body">
    <div class="table-responsive">
      <div class="my-2"></div>
      <div class="container">
<div class='notice notice-danger' style='padding-left: 20px; padding-right : 20px;'><strong>Maaf</strong> Untuk saat ini stok opname belum bisa dilakukan
    </div><br>
       <div class="form-group row">
    <label class="col-sm-2 col-form-label" style="text-align: right;">Pilih Barang</label>
    <div class="col-md-8">
